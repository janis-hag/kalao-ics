#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Filename : telmetry.py
# @Date : 2021-03-18-10-02
# @Project: KalAO-ICS
# @AUTHOR : Janis Hagelberg, Nathanael Restori
"""
The telemetry package contains the tools to store the Adaptive Optics telemetry of KalAO.

"""
from typing import Any

import numpy as np

from kalao.cacao import toolbox

import config


def gather() -> dict[str, Any]:
    telemetry_data = {}

    # Nuvu stream
    nuvu_raw_stream = toolbox.open_stream_once(config.Streams.NUVU_RAW)

    if nuvu_raw_stream is not None:
        stream_keywords = nuvu_raw_stream.get_keywords()

        telemetry_data['wfs_temp_ccd'] = stream_keywords.get('T_CCD', np.nan)
        telemetry_data['wfs_temp_controller'] = stream_keywords.get(
            'T_CNTRLR', np.nan)
        telemetry_data['wfs_temp_powersupply'] = stream_keywords.get(
            'T_PSU', np.nan)
        telemetry_data['wfs_temp_fpga'] = stream_keywords.get('T_FPGA', np.nan)
        telemetry_data['wfs_temp_heatsink'] = stream_keywords.get(
            'T_HSINK', np.nan)
        telemetry_data['wfs_emgain'] = stream_keywords.get('EMGAIN', np.nan)
        telemetry_data['wfs_detgain'] = stream_keywords.get('DETGAIN', np.nan)
        telemetry_data['wfs_exposuretime'] = stream_keywords.get(
            'EXPTIME', np.nan)
        telemetry_data['wfs_framerate'] = stream_keywords.get('MFRATE', np.nan)

    # Nuvu process
    nuvu_fps = toolbox.open_fps_once(config.FPS.NUVU)

    if nuvu_fps is not None and nuvu_fps.run_runs():
        telemetry_data['wfs_autogain_on'] = nuvu_fps.get_param('autogain_on')
        telemetry_data['wfs_autogain_setting'] = nuvu_fps.get_param(
            'autogain_setting')

    # BMC process
    bmc_fps = toolbox.open_fps_once(config.FPS.BMC)

    if bmc_fps is not None and bmc_fps.run_runs():
        telemetry_data['dm_max_stroke'] = bmc_fps.get_param('max_stroke')
        telemetry_data['dm_stroke_mode'] = bmc_fps.get_param('stroke_mode')
        telemetry_data['dm_target_stroke'] = bmc_fps.get_param('target_stroke')

    # SHWFS process
    shwfs_fps = toolbox.open_fps_once(config.FPS.SHWFS)

    if shwfs_fps is not None and shwfs_fps.run_runs():
        telemetry_data['wfs_algorithm'] = shwfs_fps.get_param('algorithm')
        telemetry_data['wfs_flux_avg'] = shwfs_fps.get_param('flux_avg')
        telemetry_data['wfs_flux_max'] = shwfs_fps.get_param('flux_max')
        telemetry_data['wfs_residual_rms'] = shwfs_fps.get_param(
            'residual_rms')

    # Tip/tilt stream
    ttm_stream = toolbox.open_stream_once(config.Streams.TTM)

    if ttm_stream is not None:
        # Check turned off to prevent timeout. Data may be obsolete
        tt_data = ttm_stream.get_data(check=False)

        telemetry_data['ttm_tip'] = float(tt_data[0])
        telemetry_data['ttm_tilt'] = float(tt_data[1])

    # DM loop process
    dmloop_fps = toolbox.open_fps_once(config.FPS.DMLOOP)

    if dmloop_fps is not None and dmloop_fps.run_runs():
        telemetry_data['dmloop_gain'] = dmloop_fps.get_param('loopgain')
        telemetry_data['dmloop_mult'] = dmloop_fps.get_param('loopmult')
        telemetry_data['dmloop_on'] = dmloop_fps.get_param('loopON')

    # TTM loop process
    ttmloop_fps = toolbox.open_fps_once(config.FPS.TTMLOOP)

    if ttmloop_fps is not None and ttmloop_fps.run_runs():
        telemetry_data['ttmloop_gain'] = ttmloop_fps.get_param('loopgain')
        telemetry_data['ttmloop_mult'] = ttmloop_fps.get_param('loopmult')
        telemetry_data['ttmloop_on'] = ttmloop_fps.get_param('loopON')

    return telemetry_data
