...
def wait_stream(stream):
    wait_loop(
            f"Waiting {stream}", lambda: not os.path.exists(
                    f"{os.environ['MILK_SHM_DIR']}/{stream}.im.shm"))
...
def init_hardware(workdir):
    ...
    subprocess.run([
            "milk-fpsinit", "-e", "cacao", "-C", "-f",
            "/home/kalao/KalAO/src/cacao/src/KalAO_BMC/fpslist.txt"
    ])
    subprocess.run([
            "milk-fpsinit", "-e", "cacao", "-C", "-f",
            "/home/kalao/KalAO/src/cacao/src/KalAO_Nuvu/fpslist.txt"
    ])
    subprocess.run([
            "milk-fpsinit", "-e", "cacao", "-C", "-f",
            "/home/kalao/KalAO/src/cacao/src/KalAO_SHWFS/fpslist.txt"
    ])
