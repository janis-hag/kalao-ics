...
def fake_fli_view():
    ...
    image = np.random.randint(0, high=np.iinfo(np.uint16).max, size=(256, 256),
                              dtype=np.uint16)
...
def fake_streams():
    ...
    for i in range(11):
        for j in range(11):
            nuvu_stream[(5 * i + 2 + 4) * cols + (5 * j + 2 + 4)] = (
                    flux[i, j] *
                    (max - min) - noise) * 1 + noise + random.gauss(0, noise)
            nuvu_stream[(5 * i + 2 + 4) * cols + (5 * j + 3 + 4)] = (
                    flux[i, j] *
                    (max - min) - noise) * 1 + noise + random.gauss(0, noise)
            nuvu_stream[(5 * i + 3 + 4) * cols + (5 * j + 2 + 4)] = (
                    flux[i, j] *
                    (max - min) - noise) * 1 + noise + random.gauss(0, noise)
            nuvu_stream[(5 * i + 3 + 4) * cols + (5 * j + 3 + 4)] = (
                    flux[i, j] *
                    (max - min) - noise) * 1 + noise + random.gauss(0, noise)

            nuvu_stream[(5 * i + 1 + 4) * cols +
                        (5 * j + 2 +
                         4)] = (flux[i, j] * (max - min) -
                                noise) * 0.1 + noise + random.gauss(0, noise)
            nuvu_stream[(5 * i + 1 + 4) * cols +
                        (5 * j + 3 +
                         4)] = (flux[i, j] * (max - min) -
                                noise) * 0.1 + noise + random.gauss(0, noise)

            nuvu_stream[(5 * i + 2 + 4) * cols +
                        (5 * j + 1 +
                         4)] = (flux[i, j] * (max - min) -
                                noise) * 0.1 + noise + random.gauss(0, noise)
            nuvu_stream[(5 * i + 3 + 4) * cols +
                        (5 * j + 1 +
                         4)] = (flux[i, j] * (max - min) -
                                noise) * 0.1 + noise + random.gauss(0, noise)
    ...
    for i in range(rows):
        for j in range(cols):
            shwfs_slopes_flux[i * cols +
                              j] = (flux[i, j] * (max - min) -
                                    noise) * 1 + min + noise + random.gauss(
                                            0, noise)
    ...
    for i in range(rows):
        for j in range(cols // 2):
            ...
            shwfs_slopes[i * cols + j] = (max - min - noise) * (
                    i / max_x) + min + noise + random.gauss(0, noise)
            shwfs_slopes[i * cols + j + cols // 2] = (max - min - noise) * (
                    j / max_y) + min + noise + random.gauss(0, noise)
        ...
        for j in range(cols):
            ...
            for subap in get_subapertures_around_actuator(act_index):
                if get_subaperture_2d(subap) != (None, None):
                    max_flux = builtins.max(max_flux,
                                            flux[get_subaperture_2d(subap)])

            if max_flux < min_flux:
                dm01disp[i * cols + j] = max
            else:
                dm01disp[i * cols + j] = (max - min - noise) * (math.sqrt(
                        (i - middle_x)**2 +
                        (j - middle_y)**2) / diag) + min + noise + random.gauss(
                                0, noise)
...
def fake_telemetry_for_db():
    ...
    telemetry["nuvu_temp_ccd"] = {
            "time_utc": [time_utc],
            "values": [-60 + random.gauss(0, 0.05)]
    }
    telemetry["nuvu_temp_controller"] = {
            "time_utc": [time_utc],
            "values": [45 + random.gauss(0, 0.05)]
    }
    ...
    telemetry["nuvu_temp_fpga"] = {
            "time_utc": [time_utc],
            "values": [50 + random.gauss(0, 0.05)]
    }
    telemetry["nuvu_temp_heatsink"] = {
            "time_utc": [time_utc],
            "values": [15 + random.gauss(0, 0.05)]
    }
    ...
    telemetry["slopes_flux_subaperture"] = {
            "time_utc": [time_utc],
            "values": [2**16 - 1 - 200 + random.gauss(0, 200)]
    }
    telemetry["slopes_residual"] = {
            "time_utc": [time_utc],
            "values": [0.05 + random.gauss(0, 0.02)]
    }

    telemetry["pi_tip"] = {
            "time_utc": [time_utc],
            "values": [0 + random.gauss(0, 0.5)]
    }
    telemetry["pi_tilt"] = {
            "time_utc": [time_utc],
            "values": [0 + random.gauss(0, 0.5)]
