...
def rotate(adc_id, position=0):
    ...
    init_result = initialise(adc_id, force_init=False, beck=beck, motor_nCommand=motor_nCommand,
                             motor_bExecute=motor_bExecute)
    if not init_result == 0:
        return init_result

    # Set velocity to 1 in case is has been changed
    motor_lrVelocity = beck.get_node("ns=4; s=MAIN." + adc_name[adc_id] + ".ctrl.lrVelocity")
    ...
    if isinstance(position, numbers.Number):
        ...
        while beck.get_node("ns=4; s=MAIN." + adc_name[adc_id] + ".stat.sStatus").get_value() == 'MOVING in Positioning Mode':
            print('.')
            sleep(5)
        # Get new position
        new_position = beck.get_node("ns=4; s=MAIN." + adc_name[adc_id] + ".stat.lrPosActual").get_value()
        # motor_lrPosition = beck.get_node("ns=4; s=MAIN." + adc_name[adc_id] + ".ctrl.lrPosition")
    else:
...
def initialise(adc_id, force_init=False, beck=None, motor_nCommand=None, motor_bExecute=None):
    ...
    if not beck.get_node("ns=4; s=MAIN." + adc_name[adc_id] + ".stat.bEnabled").get_value() or force_init:
        motor_bEnable = beck.get_node("ns = 4; s = MAIN." + adc_name[adc_id] + ".ctrl.bEnable")
