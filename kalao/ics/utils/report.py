...
def generate(since: datetime, until: datetime, short: bool = False,
    ...
    with io.StringIO() as str_io:
        ...
        keys = sorted(list(database_definitions.monitoring.keys()))
        pipeline = [{
            '$match': {
                'metadata.key': {
                    '$in': keys
        ...
        for key in keys:
            ...
            if key in data:
                ...
                for row in data[key]:
                    alarm = database_definitions.check_alarm(key, row['value'])
            ...
            monitoring_stats.append({
                'key': key,
                'short': database_definitions.monitoring[key]['short'],
                'points': points,
                'warnings': warnings,
                'alarms': alarms
            })
        ...
        keys = sorted(list(database_definitions.logs.keys()))
        pipeline = [{
            '$match': {
                'metadata.key': {
                    '$in': keys
