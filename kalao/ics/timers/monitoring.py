...
from kalao.common.enums import AlarmLevel, CameraServerStatus, LoopStatus

from kalao.ics import database, euler, ippower, logger, memory, services
from kalao.ics.cacao import aocontrol, toolbox
from kalao.ics.hardware import adc, camera, hw_utils, ttm
from kalao.ics.rtc import gpu, sensors
...
def _update_general() -> None:
    ...
    for key in data.keys():
        ...
        alarm = database_definitions.check_alarm(key, data[key])
...
def _round(key: str, value: Any):
    metadata = database_definitions.monitoring[key]
