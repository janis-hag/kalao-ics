...
class gop():
    ...
    def initializeGopConnection(self, myName, verbosity):
        ...
        if status != 0:
            print("%.6f" % (time.time()), "gop_accept_connection: status = ",
                  status, "  gop_errno = ", pymod_libgop.cvar.gop_errno, " : ",
                  pymod_libgop.gop_get_error_str())
        #
...
#-------------------------------------------------------------------------------
    ...
    def initializeClientGopConnection(self, myName, verbosity):
        ...
        if status != 0:
            print("%.6f" % (time.time()), "gop_connection: status = ", status,
                  "  gop_errno = ", pymod_libgop.cvar.gop_errno, " : ",
                  pymod_libgop.gop_get_error_str())
...
#-------------------------------------------------------------------------------
    ...
    def read(self):
        ...
        if returnList[0] <= 0:
            print("%.6f" % (time.time()),
                  "ERROR during pymod_libgop.gop_read(): status =", returnList,
                  " gop_errno =", gop.getErrNo(self), ":", gop.getErrStr(self))
            return (-1)
        print("%.6f" % (time.time()), " Recu: ", returnList[0],
              " bytes, answer = >", returnList[1], "<", sep="")
        return returnList[1]
...
#-------------------------------------------------------------------------------
    ...
    def write(self, string):
        ...
        if status != 0:
            print("%.6f" % (time.time()), "gop_write_command: status = ",
                  status, "  gop_errno = ", pymod_libgop.cvar.gop_errno, " : ",
                  pymod_libgop.gop_get_error_str())
