...
def initializeGopConnection():
    ...
    if status != 0:
        print("gop_accept_connection: status = ", status, "  gop_errno = ",
              pymod_libgop.cvar.gop_errno, " : ",
...
if nbBytes == -1:
    print("gop_read_data: status = ", nbBytes, "  gop_errno = ",
          pymod_libgop.cvar.gop_errno, " : ", pymod_libgop.gop_get_error_str())

print("recu: ", nbBytes, " bytes, answer = >", answer, "<")

status = pymod_libgop.gop_write_command(gc, "OK")
if status != 0:
    print("gop_write_command: status = ", status, "  gop_errno = ",
          pymod_libgop.cvar.gop_errno, " : ", pymod_libgop.gop_get_error_str())
